...
def log_function_call(logger_name=None, log_args=True, log_result=True, category=Categories.PERFORMANCE):
    """
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            logger = Categories.get_logger(logger_name or f"{func.__module__}.{func.__name__}", category)
...
def log_exceptions(logger_name=None, reraise=True):
    ...
    def decorator(func):
        @functools.wraps(func)
        def wrapper(*args, **kwargs):
            logger = Categories.get_logger(logger_name or f"errors.{func.__name__}", Categories.ERRORS)
