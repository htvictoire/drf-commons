...
class LibraryInstallationTests(TestCase):
    ...
    def test_main_package_importable(self):
        """Test main drf_common package can be imported."""
        try:
            import drf_common
            self.assertTrue(hasattr(drf_common, '__version__'))
        except ImportError:
            self.fail("drf_common package could not be imported")
    ...
    def test_all_public_modules_importable(self):
        ...
        public_modules = [
            ...
            'drf_common.debug',
            'drf_common.decorators',
            'drf_common.filters',
            'drf_common.middlewares',
            'drf_common.models',
    ...
    def test_public_api_imports(self):
        ...
        api_imports = [
            # Views
            ('drf_common.views.base', 'BaseViewSet'),
            ('drf_common.views.mixins', 'BulkCreateModelMixin'),
            ('drf_common.views.mixins', 'FileImportMixin'),

            # Serializers
            ('drf_common.serializers.fields.custom', 'FlexibleField'),

            # Models
            ('drf_common.models.base', 'AbstractBaseModel'),

            # Services
            ('drf_common.services.import_from_file.service', 'FileImportService'),
            ('drf_common.services.export_file.service', 'ExportService'),

            # Middlewares
            ('drf_common.middlewares.current_user', 'CurrentUserMiddleware'),
            ('drf_common.middlewares.debug', 'DebugMiddleware'),
        ]
    ...
    def test_optional_dependencies_handling(self):
        ...
        with patch.dict(sys.modules, {'openpyxl': None}):
            try:
                from drf_common.services.export_file.exporters import xlsx_exporter
        ...
        with patch.dict(sys.modules, {'reportlab': None}):
            try:
                from drf_common.services.export_file.exporters import pdf_exporter
                # Should not fail during import, only during usage
            except ImportError:
    ...
    def test_settings_module_exists(self):
        ...
        try:
            from drf_common.common_conf import settings
            self.assertTrue(hasattr(settings, 'IMPORT_BATCH_SIZE'))
        except ImportError:
            self.fail("Could not import drf_common settings")

        try:
            import drf_common.common_conf.django_settings
    ...
    def test_common_tests_utilities_importable(self):
        ...
        test_utilities = [
            'drf_common.common_tests.base_cases',
            'drf_common.common_tests.factories',
            'drf_common.common_tests.utils',
            'drf_common.common_tests.serializers',
        ]
...
class ModularUsageTests(TestCase):
    ...
    def test_current_user_standalone(self):
        ...
        from drf_common.current_user.utils import get_current_user
    ...
    def test_decorators_standalone(self):
        ...
        from drf_common.decorators.cache import cache_debug
    ...
    def test_response_utils_standalone(self):
        ...
        from drf_common.response.utils import success_response, error_response
    ...
    def test_models_standalone(self):
        ...
        from drf_common.models.mixins import TimeStampMixin
    ...
    def test_templatetags_standalone(self):
        ...
        from drf_common.templatetags.dict_extras import get_item
...
class CompatibilityTests(TestCase):
    ...
    def test_import_with_minimal_django_settings(self):
        ...
        required_modules = [
            'drf_common.models.base',
            'drf_common.serializers.fields.base',
            'drf_common.views.base',
        ]
