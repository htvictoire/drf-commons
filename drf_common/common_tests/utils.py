"""
Test utilities for DRF Common library tests.
"""
...
from drf_common.current_user.utils import USER_ATTR_NAME, _set_current_user, _thread_locals, get_current_user
from drf_common.common_conf.settings import clear_settings_cache
...
def assert_response_success(response, expected_status=200):
    """Assert response is successful with DRF Common format."""
...
def assert_response_error(response, expected_status=400):
    """Assert response is error with DRF Common format."""
    assert response.status_code == expected_status
    assert "message" in response.data
    assert "success" in response.data
...
def override_debug_settings(**settings_dict):
    """Override DRF Common debug settings for testing."""
