...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            columns_ordered, column_required_status = self.extract_columns_from_config(viewset_class.import_file_config)
            ...
            if not filename:
                if hasattr(viewset_class, 'import_template_name') and viewset_class.import_template_name:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"✅ Template created successfully: {template_path}"
                )
            )
            
            # Show column summary with color coding info
            required_count = sum(1 for is_req in column_required_status.values() if is_req)
            ...
            if order_by == 'alphabetic':
                ...
                required_cols = [col for col in columns_ordered if column_required_status.get(col, True)]
                optional_cols = [col for col in columns_ordered if not column_required_status.get(col, True)]
            ...
            # Show usage info
            self.stdout.write(f"\n📖 Usage:")
            self.stdout.write(f"   1. Download template: GET /api/.../download-import-template/")
            self.stdout.write(f"   2. Fill in your data following the column headers")
            self.stdout.write(f"   3. Upload via: POST /api/.../import-from-file/")
    ...
    def extract_columns_from_config(self, config: dict) -> tuple[List[str], Dict[str, bool]]:
        ...
        for step_key in config["order"]:
            ...
            self.stdout.write(f"📋 Processing step '{step_key}' (model: {model_config.get('model', 'Unknown')})")
            ...
            if "direct_columns" in model_config:
                for field_name, column_name in model_config["direct_columns"].items():
                    if column_name not in column_required_status:  # Avoid duplicates
                        ...
                        is_required = self._is_field_required(model_config["model"], field_name, model_config)
                        column_required_status[column_name] = is_required
                        status = "required" if is_required else "optional"
                        self.stdout.write(f"  ✓ Added column: '{column_name}' (direct, {status})")
            ...
            if "transformed_columns" in model_config:
                for field_name, transform_spec in model_config["transformed_columns"].items():
                    ...
                    if column_name not in column_required_status:  # Avoid duplicates
                        ...
                        column_required_status[column_name] = is_required
                        status = "required" if is_required else "optional"
                        self.stdout.write(f"  ✓ Added column: '{column_name}' (transformed, {status})")
            ...
            if "lookup_fields" in model_config:
                for field_name, lookup_spec in model_config["lookup_fields"].items():
                    ...
                    if column_name not in column_required_status:  # Avoid duplicates
                        ...
                        column_required_status[column_name] = is_required
                        status = "required" if is_required else "optional"
                        self.stdout.write(f"  ✓ Added column: '{column_name}' (lookup, {status})")
                ...
                for field_name, compute_spec in model_config["computed_fields"].items():
                    ...
                    if mode == "if_empty" and "column" in compute_spec:
                        ...
                        if column_name not in column_required_status:  # Avoid duplicates
                            ...
                            self.stdout.write(f"  ✓ Added column: '{column_name}' (computed-hybrid, optional)")
                    elif mode == "always":
                        # Fully generated field: no column needed
                        self.stdout.write(f"  ⚡ Skipped field: '{field_name}' (computed-always, no column needed)")
        
        return columns_ordered, column_required_status

    def _is_field_required(self, model_path: str, field_name: str, model_config: Dict[str, Any] = None) -> bool:
    ...
    def create_template_file(
        ...
        if format_type == 'xlsx':
            ...
            # Style legend
            from openpyxl.styles import Font as LegendFont
            legend_font = LegendFont(bold=True, size=10)
            ...
            required_fill = PatternFill(start_color="DC3545", end_color="DC3545", fill_type="solid")  # Red for required
            
            optional_font = Font(bold=True, color="FFFFFF") 
            optional_fill = PatternFill(start_color="28A745", end_color="28A745", fill_type="solid")  # Green for optional
            
            for cell in ws[5]:  # Header row (after legend)
                column_name = cell.value
                is_required = column_required_status.get(column_name, True)  # Default to required if unknown
