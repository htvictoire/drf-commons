...
from constance import config as school_settings
from .utils import extract_nested_value


def extract_common_values_and_filter_columns(data: List[Dict], includes: List[str], 
                                             column_config: Dict[str, Dict]) -> Tuple[Dict[str, str], List[str]]:
    """
    ...
    for field_name in includes:
        ...
        if field_config.get('can_be_common', False):
            ...
            unique_values = set(str(v) for v in non_empty_values)  # Convert to string for comparison
            
            # If all non-empty values are the same, we have at least one value, and all records have this value
            if len(unique_values) == 1 and len(non_empty_values) > 0 and len(non_empty_values) == len(data):
...
def prepare_export_headers(common_values: Dict[str, str]) -> List[str]:
    """
    Prepare export headers combining school header and common values only.
    ...
    # Add school header from settings
    school_header_str = getattr(school_settings, 'SCHOOL_DEFAULT_DOCS_HEADER')
    school_header = [line.strip() for line in school_header_str.split(',')] if school_header_str else []
    headers.extend(school_header)
    
    # Add empty line after school header if we have school header
    if school_header and common_values:
        headers.append('')
...
def process_export_data(provided_data: List[Dict], includes: List[str], 
                       column_config: Dict[str, Dict], file_titles: List[str] = None) -> Dict[str, Any]:
    """
    ...
    # Prepare export headers (school header + common values only)
