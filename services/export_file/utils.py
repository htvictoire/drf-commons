...
from utils.values import get_working_date # conveniance import for other export files importing other utils from here
...
def extract_nested_value(data: Dict[str, Any], field_path: str) -> Any:
    ...
    if current is not None and not isinstance(current, (str, int, float, bool, type(None))):
        return str(current)
